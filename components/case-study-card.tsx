    ...
    <article className="group overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] transition hover:-translate-y-1 hover:border-white/20">
      ...
      <div className="space-y-4 p-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs uppercase tracking-[0.18em] text-white/55">{caseStudy.client}</span>
        </div>

        <h3 className="text-2xl font-semibold tracking-tight text-white">{caseStudy.title}</h3>
        <p className="text-sm leading-relaxed text-white/75">{caseStudy.problem}</p>

        {isDetailed ? (
          <>
            <div className="rounded-2xl border border-emerald-200/20 bg-emerald-200/10 p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-emerald-200">Key result</p>
              <p className="mt-2 text-sm font-medium text-white">{caseStudy.keyResult}</p>
            </div>

            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-white/55">Services used</p>
